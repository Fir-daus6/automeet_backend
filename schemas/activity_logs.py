...
def _normalize_datetimes_to_iso(value: Any) -> Any:
    if isinstance(value, (datetime, date)):
        return value.isoformat()
    if isinstance(value, dict):
        return {k: _normalize_datetimes_to_iso(v) for k, v in value.items()}
    if isinstance(value, list):
        return [_normalize_datetimes_to_iso(item) for item in value]
    return value
...
class ActivityLogBaseSchema(BaseModel):
    ...
    @classmethod
    def convert_datetime_to_isoformat(cls, values: Any) -> Any:
        if isinstance(values, dict):
            return {k: _normalize_datetimes_to_iso(v) for k, v in values.items()}
        return values
...
class ActivityLogFilters(BaseFilters):
    ...
    search: Optional[str] = Field(
        default=None, description="Search logs by entity, action, description, or user details"
    )
