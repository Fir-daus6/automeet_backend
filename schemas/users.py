...
class AdminUserCreateSchema(UserBaseSchema):
    """Admin bulk user create: comma-separated ``role_uuid`` for RBAC roles."""

    role_uuid: str = Field(..., description="Comma-separated role UUIDs to assign")
    is_active: Optional[bool] = Field(True, description="Whether the user account is active")


class AdminSendEmailSchema(BaseModel):
    email: EmailStr = Field(..., description="User email to resend initialization link to")
...
class UserFilters(BaseFilters):
    ...
    user_type: Optional[str] = Field(
        None, description="Comma-separated RBAC role names to include"
    )
    exclude_user_types: Optional[str] = Field(
        None, description="Comma-separated RBAC role names to exclude"
    )
    has_dashboard_access: Optional[bool] = Field(
        None, description="Filter users whose RBAC roles include dashboard access"
    )
