...
class CRUDActivityLog(
    ...
    async def create(self, db: AsyncSession, obj_in: ActivityLogCreateSchema) -> ActivityLog:
        """
        ...
        previous_changes, new_changes = await self.changes_made(
            previous_data=cast(Optional[Dict[str, Any]], db_obj.previous_data),
            new_data=cast(Optional[Dict[str, Any]], db_obj.new_data),
        )
